    ...
    void refresh(Info info, int cnt){
        ...
        for(int j = 0; j < pre_info.lengthMove; j++){
            ...
            if(flag == 0) clear_img(is_img_16(pre_info.infomove[j].tag), pre_info.infomove[j].x_dest, pre_info.infomove[j].y_dest);
            /*
            擦除判断应该直接统一进画图中
            else{
    ...
    void draw_move(InfoMove infoMove, int time){
        ...
        else clear_img(infoMove.tag, x, y);     //可能会导致移动中的物体显示时间太短了
    };
